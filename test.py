...
* **`save_tasks()`:**
    ...
    * Uses `json.dump` to serialize the task list into JSON format.
* **`display_tasks()`:**
    * Displays the list of tasks in a formatted way.
    * If the task list is empty, it prints a message indicating that there are no tasks.
    * For each task, it prints the index, completion status (✓ for completed, ✗ for incomplete), and task title.
* **`add_task()`:**
    * Appends a new task to the list.
    * The new task is a dictionary with keys `title` and `completed` (initially set to `False`).
    * Calls `save_tasks` to persist the updated task list.
* **`remove_task()`:**
    * Removes a task from the list based on its index.
    * Checks if the index is valid.
    * If valid, removes the task using `tasks.pop` and saves the updated list using `save_tasks`.
    * If invalid, prints an error message.
* **`complete_task()`:**
    * Marks a task as completed based on its index.
    * Checks if the index is valid.
    * If valid, sets the `completed` status of the task to `True` and saves the updated list.
    * If invalid, prints an error message.
* **`main()`:**
    * The main function that handles user interactions.
    * Loads tasks from the file.
    * Enters a loop that continues until the user chooses to exit.
    * Displays the menu options.
    * Prompts the user to select an option.
    * Based on the choice, calls the appropriate function (e.g., `display_tasks`, `add_task`, `remove_task`, `complete_task`).
    * If the user chooses to exit, prints a goodbye message and breaks the loop.
